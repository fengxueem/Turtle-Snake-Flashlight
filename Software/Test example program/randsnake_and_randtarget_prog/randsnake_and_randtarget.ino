...
void loop()
{
  // put your main code here, to run repeatedly:
    for(int i=0;i<=60;i++)
    {
     ...
     randsnake(); randtarget();lightone(5,5,500);
}
...
void randtarget(void)
{
  do
  {
	  randx=(rand()%9)+1;
	  randy=(rand()%9)+1;
  }while((abs(randx-randy)>=5)||(randx<1)||(randx>9)||(randy<1)||(randy>9)||((randx==x_1)&&(randy==y_1))||((randx==x_2)&&(randy==y_2))||((randx==x_3)&&(randy==y_3)));
}
...
void randsnake(void)
{
  do
  {
	x_1=(rand()%5)+3;
        y_1=(rand()%5)+3;
  }while((abs(x_1-y_1)>=3)||(x_1<3)||(x_1>7)||(y_1<3)||(y_1>7));//control the area where the snake's head  first shows up into central haxegon with side length equal to 3 leds
  do
  {
    d=rand()%6;
  }while((d<0)||(d>6));
